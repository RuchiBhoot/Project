...
parti = "dbh"
#graph = "test"
worker1 = "triangle/edge/"+graph+"/"+parti+"/partition-0.txt"
worker2 = "triangle/edge/"+graph+"/"+parti+"/partition-1.txt"
worker3 = "triangle/edge/"+graph+"/"+parti+"/partition-2.txt"
worker4 = "triangle/edge/"+graph+"/"+parti+"/partition-3.txt"
worker5 = "triangle/edge/"+graph+"/"+parti+"/partition-4.txt"
worker6 = "triangle/edge/"+graph+"/"+parti+"/partition-5.txt"
worker7 = "triangle/edge/"+graph+"/"+parti+"/partition-6.txt"
worker8 = "triangle/edge/"+graph+"/"+parti+"/partition-7.txt"
'''
worker1 = "triangle/edge/worker0.txt"
worker2 = "triangle/edge/worker1.txt"
worker3 = "triangle/edge/worker2.txt"
worker4 = "triangle/edge/worker2.txt"
worker5 = "triangle/edge/worker2.txt"
worker6 = "triangle/edge/worker2.txt"
worker7 = "triangle/edge/worker2.txt"
worker8 = "triangle/edge/worker2.txt"
'''
w1rdd = sc.textFile(worker1)
w1rdd = w1rdd.map(lambda x: x.split(' ')).map(lambda x: [int(x[0]),[int(x[1])]])
#print(w1rdd.count())
w1rdd = w1rdd.filter(lambda x : x[0]%15==0 and x[1][0]%15==0)
#print(w1rdd.count())
w2rdd = sc.textFile(worker2)
w2rdd = w2rdd.map(lambda x: x.split(' ')).map(lambda x: [int(x[0]),[int(x[1])]])
w2rdd = w2rdd.filter(lambda x : x[0]%15==0 and x[1][0]%15==0)
w3rdd = sc.textFile(worker3)
w3rdd = w3rdd.map(lambda x: x.split(' ')).map(lambda x: [int(x[0]),[int(x[1])]]).filter(lambda x : x[0]%15==0 and x[1][0]%15==0)
w4rdd = sc.textFile(worker4)
w4rdd = w4rdd.map(lambda x: x.split(' ')).map(lambda x: [int(x[0]),[int(x[1])]]).filter(lambda x : x[0]%15==0 and x[1][0]%15==0)
w5rdd = sc.textFile(worker5)
w5rdd = w5rdd.map(lambda x: x.split(' ')).map(lambda x: [int(x[0]),[int(x[1])]]).filter(lambda x : x[0]%15==0 and x[1][0]%15==0)
w6rdd = sc.textFile(worker6)
w6rdd = w6rdd.map(lambda x: x.split(' ')).map(lambda x: [int(x[0]),[int(x[1])]]).filter(lambda x : x[0]%15==0 and x[1][0]%15==0)
w7rdd = sc.textFile(worker7)
w7rdd = w7rdd.map(lambda x: x.split(' ')).map(lambda x: [int(x[0]),[int(x[1])]]).filter(lambda x : x[0]%15==0 and x[1][0]%15==0)
w8rdd = sc.textFile(worker8)
w8rdd = w8rdd.map(lambda x: x.split(' ')).map(lambda x: [int(x[0]),[int(x[1])]]).filter(lambda x : x[0]%15==0 and x[1][0]%15==0)
print("here 1")
#print(w1rdd.collect())
